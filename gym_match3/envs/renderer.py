...
class Renderer:
    ...
    def render_board(self, board: Board):
        ...
        plt.xticks(
            ticks=np.arange(1, cols + 1, step=1), labels=np.arange(0, cols, step=1)
        )
        plt.yticks(
            ticks=np.arange(1, rows + 1, step=1), labels=np.arange(0, rows, step=1)
        )
        ax.set_yticklabels(labels=np.arange(0, rows, step=1)[::-1])
        ax.xaxis.tick_top()
        plt.grid(which="minor", linewidth=2, c="white")
        plt.imshow(
            board.board, extent=[0.5, 0.5 + cols, 0.5, 0.5 + rows], cmap=cmap, norm=norm
        )
